...
export const install: LocalizedPage<InstallContent> = {
  en: {
    ...
    terminal: {
      ...
      branchNote: "The script is served from main, keel's default branch. It has never been part of a tagged release, so there is no version of it to name, and it changes when main changes: the Python floor above moved from 3.11 to 3.14 that way. Read the copy you download rather than trusting this description to have kept up. One thing that floor costs you today — Python 3.14 is recent enough that most Linux distributions do not package it yet, and keel's own Linux smoke run fails at exactly that check. macOS passes it with a Homebrew Python. On Linux, expect to install 3.14 yourself before this script will run.",
  ...
  ar: {
    ...
    terminal: {
      ...
      branchNote: "ويُقدَّم السكربتُ من الفرع ‏main، وهو الفرع الافتراضي لمستودع كيل. ولم يكن يومًا جزءًا من إصدارٍ موسوم، فليس له رقمُ إصدارٍ نشير إليه، وهو يتغيّر كلّما تغيّر ‏main: فبهذا الطريق انتقل حدُّ ‏Python أعلاه من 3.11 إلى 3.14. فاقرأ النسخة التي تنزّلها، ولا تفترض أنّ هذا الوصف قد واكب التغيير. وثمّة ثمنٌ يدفعه هذا الحدُّ اليوم — فنسخة ‏Python 3.14 حديثةٌ إلى حدٍّ لم تحزمها بعدُ معظمُ توزيعات ‏Linux، ولذلك يفشل اختبارُ كيل الدخانيُّ على ‏Linux عند هذا الفحص بعينه. أمّا ‏macOS فيجتازه بنسخة ‏Python من ‏Homebrew. فعلى ‏Linux توقّع أن تثبّت 3.14 بنفسك قبل أن يعمل هذا السكربت.",
  ...
  fr: {
    ...
    terminal: {
      ...
      branchNote: "Le script est servi depuis main, la branche par défaut de keel. Il n'a jamais fait partie d'une version publiée, il n'y a donc aucun numéro à citer, et il change quand main change : c'est ainsi que le seuil Python ci-dessus est passé de 3.11 à 3.14. Lisez la copie que vous téléchargez, plutôt que de supposer que cette description a suivi. Ce seuil a un coût aujourd'hui — Python 3.14 est assez récent pour que la plupart des distributions Linux ne le proposent pas encore, et le test de fumée de keel sur Linux échoue précisément à cette vérification. macOS le franchit avec un Python de Homebrew. Sous Linux, attendez-vous à installer 3.14 vous-même avant que ce script ne démarre.",
